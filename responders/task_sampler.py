...
class TaskSamplerResponder:
    ...
    def act(self, obs: Observation) -> Action:
        ...
        if not keys:
            return Action(key=None, rt_s=None, meta={"source": "lexical_sampler", "reason": "no_valid_keys"})
        factors = dict(getattr(obs, "task_factors", {}) or {})
        stage = str(factors.get("stage", getattr(obs, "phase", "")))
        if any(token in stage for token in ("instruction", "summary", "good_bye", "intro")):
        ...
        if trial_id in set(self.forced_timeout_trials):
            return Action(key=None, rt_s=None, meta={"source": "lexical_sampler", "outcome": "forced_timeout"})
        ...
        return Action(key=response_key, rt_s=self._rt(), meta={"source": "lexical_sampler", "correct": not make_error})
